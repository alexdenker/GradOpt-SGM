...
class Energy_UNetModel_full(nn.Module):
    ...
    def forward(self, x, timesteps,eval=False, only_energy=False):
        """
        ...
        #energy_norm_ = 0.5 * (energy_score ** 2) / self.marginal_prob_std(timesteps)[:, None, None, None]
        #energy_norm = energy_norm_.sum()

        # Energy Score = ||f(x)||^2  l2 norm squared
        energy_norm_ = - 0.5 * (unet_out** 2) / self.marginal_prob_std(timesteps)[:, None, None, None]
        energy_norm = energy_norm_.sum()
        if only_energy:
            return energy_norm_
