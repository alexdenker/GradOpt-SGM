...
class EllipsesDataset(torch.utils.data.Dataset):
    ...
    normalise: 'bool'
        If the image should be normalised to have a maximum value of 1
    """

    def __init__(self, shape=[1,128,128], n_samples = 100, mode="train", seed = 1, normalise=False):
        self.n_samples = n_samples
        self.shape = shape
        self.normalise = normalise
        if mode == 'valid':
            self.x_gt = shepp_logan(self.shape)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            x_gt = random_phantom(self.shape)
        ...
        return torch.from_numpy(x_gt).float()
